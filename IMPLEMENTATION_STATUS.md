...
- Agent 注册与 HTTP 代理调用
- MCP 三级配置实时合并、HTTP/stdio JSON-RPC 转发、远程工具 120 秒缓存
...
- cron：每 4 小时语义同步；每天 02:00 完整备份（文件 + PostgreSQL pg_dump，实机已验证）
...
- 本地：ruff 通过，11/11 pytest 通过，wheel/sdist 构建通过
- UG：health/register/memory/wiki/whoosh/skills/backup/ui/systemd/pgvector 实机通过

## 远端仓库

目标：`https://cnb.cool/duan_z/DHub`

当前设备没有 CNB HTTPS 凭据，尚未推送。代码已提交到本地 Git；配置 CNB token 或 SSH key 后可直接 push。
